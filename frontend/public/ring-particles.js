...
class RingParticles {
	...
	paint(ctx, geom, properties) {
		...
		for (let r = 0; r < numRows; r++) {
			...
			const colorProgress = Math.cos(rowProgress * Math.PI * 2) * 0.5 + 0.5;
			const rColor = Math.round(192 - (192 - 96) * colorProgress);
			const gColor = Math.round(132 - (132 - 165) * colorProgress);
			const bColor = Math.round(252 - (252 - 250) * colorProgress);
			...
			for (let i = 0; i < numParticles; i++) {
				...
				// --- DUAL EDGE FADE LOGIC (Inner and Outer Rings) ---
